      ...
      <div className="relative w-full h-[calc(100vh-128px)]">
        {blocks.map((block, index) => (
          <div
            ...
            <div className="absolute inset-0 flex flex-col justify-end items-start p-24 z-20 pb-80">
              <div className="max-w-2xl">
                <h1 className="font-space-grotesk text-6xl font-bold text-accent-red leading-tight mb-4">{block.title}</h1>
                {block.subtitle && <h3 className="font-space-grotesk text-3xl font-semibold text-text-main mb-4 uppercase tracking-wider">{block.subtitle}</h3>}
                <p className="text-xl text-text-gray font-medium">{block.body}</p>
              </div>
            </div>
          </div>
        ))}

        <div className="absolute bottom-16 left-24 z-20 flex items-center gap-3">
          {blocks.map((_, index) => (
            <button
              key={index}
