    ...
    <section id={section.id} className="min-h-screen bg-annexure">
      {section.sub_sections?.map((subSection) => (
        <div key={subSection.id} id={subSection.id} className="mb-16">
          <div className="sticky top-0 bg-paper px-24 py-8 z-10 border-b border-border-color h-32 flex flex-col justify-center">
            ...
            {subSection.id === 'annexure-flows' ? (
                  ...
                  if (block.type === 'text') {
                    ...
                    return (
                      <div key={index} className="mt-8">
                        ...
                        <div className="bg-grid-pattern rounded-md border border-border-color">
              ...
              subSection.blocks.map((block, index) => {
                if (block.type === 'partnership_table') {
                  return (
                    <div key={index} className="bg-card rounded-lg shadow-lg overflow-hidden border border-border-color">
                      <div className="grid grid-cols-[1.5fr,1fr,2.5fr,1.5fr] bg-sidebar p-5 font-bold uppercase text-sm text-text-gray tracking-wider">
                      ...
                      {block.rows.map((row, rowIndex) => (
                        <div key={rowIndex} className="grid grid-cols-[1.5fr,1fr,2.5fr,1.5fr] p-6 border-t border-border-color items-center">
